...
load('ADNI_FS_MIL.mat')

for W_idx = 1 : size(ADNI_FS_MIL{1}.Weight,2)
    X_RAVLT=[];
    for idx = 1 : size(ADNI_FS_MIL,2)-1
        if sum(isnan(ADNI_FS_MIL{idx}.RAVLT_BL))==0 && sum(isempty(ADNI_FS_MIL{idx}.RAVLT_BL))==0 && sum(sum(isnan(ADNI_FS_MIL{idx}.Weight{W_idx})))==0      
            X_RAVLT = [X_RAVLT;ADNI_FS_MIL{idx}.Base*ADNI_FS_MIL{idx}.Weight{W_idx}'];
        end
    end
...
X_RAVLT=[];Y_RAVLT=[];
for idx = 1 : size(ADNI_FS_MIL,2)-1
    if sum(isnan(ADNI_FS_MIL{idx}.RAVLT_BL))==0 && sum(isempty(ADNI_FS_MIL{idx}.RAVLT_BL))==0
        X_RAVLT = [X_RAVLT;ADNI_FS_MIL{idx}.Base];
        Y_RAVLT = [Y_RAVLT;ADNI_FS_MIL{idx}.RAVLT_BL];       
    end
end
 ...
 for Mdl_idx =  1 :size(X_RAVLT_W,2)
     for KFold_idx  = 1 : 5 
         ...
         Mdl_Lasso{Mdl_idx,KFold_idx,DX_idx} = fitrlinear(X_train,Y_train(:,DX_idx),'Lambda',10.^(-10:10),'Regularization','lasso');
    ...
    save('R_FS_RAVLT.mat','RMSE_SVR_avg_RAVLT','RMSE_LR_avg_RAVLT','RMSE_RR_avg_RAVLT','RMSE_Lasso_avg_RAVLT',...
